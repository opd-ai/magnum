...
- [x] **F-20: SetFrameDuration doesn't update stereo right-channel codecs** — `encoder.go:377-386` — Logic — `SetFrameDuration` updates `celtEncoder`/`silkEncoder` config but not `celtEncoderR`/`silkEncoderR`. Stereo channel 2 retains old frame size and produces mismatched packets. **Remediation**: Also update `*EncoderR` instances. Validate: stereo encode after `SetFrameDuration` change.
